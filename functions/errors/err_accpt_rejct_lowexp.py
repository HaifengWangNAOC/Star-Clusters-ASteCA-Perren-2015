...
def err_a_r_lowexp(mag, e_mag, e_col, be_m):
    '''
    ...
    popt_col, pcov_col = curve_fit(exp_func, mag, e_col)

    # Add a number of sigmas to one or more parameters of the exponential.
    sigmas_m = np.sqrt(np.diag(pcov_mag))
    sigmas_c = np.sqrt(np.diag(pcov_col))
    for i in [0, 1, 2]:
        popt_mag[i] = popt_mag[i] + float(N_sig) * sigmas_m[i]
        popt_col[i] = popt_col[i] + float(N_sig) * sigmas_c[i]

    # Use the fitted curves to identify accepted/rejected stars and store
    # their indexes.
    acpt_indx, rjct_indx = separate_stars(mag, e_mag, e_col, be_m, popt_mag,
        popt_col)

    err_plot = [popt_mag, popt_col]
